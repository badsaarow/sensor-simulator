  ...
  methods: {
    showPorts() {
      SerialPort.list()
        .then((ports) => {
          ...
          this.ports = ports.map((v) => ({
            value: v.path,
            text: `${v.path}-${v.manufacturer}`,
          }));
        })
        .catch((err) => {
    ...
    createToInt(size) {
      ...
      return (value) => {
        ...
        return value < 0 ? (1 << size) + value : value;
      };
    },
    sendEvent() {
      ...
      this.arduino.on("message", (data) => {
        ...
        if (data[0] === "#".charCodeAt(0)) {
          for (let i = 0; i < data.length - 4 && i < 29; i += 1) {
            const hexValue = data
              .slice(i * 2 + 1 + 4, i * 2 + 1 + 2 + 4)
              .toString();
            console.log(i, hexValue);
            document.getElementsByName("sensor")[i].value = parseInt(
              hexValue,
              16
            ); // 16진수로 표시
          }
